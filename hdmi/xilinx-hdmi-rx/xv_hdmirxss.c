...
/**
*
* This function prints the HDMI RX SS information to passed buffer
*
...
*
******************************************************************************/
int XV_HdmiRxSs_ShowInfo(XV_HdmiRxSs *InstancePtr, char *buff, int buff_size)
{
	int strSize = 0;
	u8 Channel;
	u32 Errors;
    ...
    if (XV_HdmiRx_IsStreamUp(InstancePtr->HdmiRxPtr)) {
		AudioFormat = XV_HdmiRxSs_GetAudioFormat(InstancePtr);
		strSize += scnprintf(buff+strSize, buff_size-strSize,
				"VIC : %0d\r\n", InstancePtr->HdmiRxPtr->Stream.Vic);
		strSize += scnprintf(buff+strSize, buff_size-strSize,
				"Scrambled: %0d\r\n",
	...
	for (Channel = 0; Channel < 3; Channel++)
	{
		Errors = XV_HdmiRx_GetLinkStatus(InstancePtr->HdmiRxPtr, Channel);
		strSize += scnprintf(buff+strSize, buff_size-strSize,
			"Link quality channel %0d : ", Channel);
