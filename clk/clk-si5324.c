...
static void si5324_initialize(struct si5324_driver_data *drvdata)
{
	...
	/* enable free-run */
	si5324_reg_write(drvdata, 0, 0x54);
    ...
    // other bits are default
	si5324_reg_write(drvdata, 19, 0x23);
	...
	si5324_reg_write(drvdata, 137, 0x01);
}
...
static int si5324_xtal_prepare(struct clk_hw *hw)
{
	...
	si5324_dbg("si5324_xtal_prepare-nop; set to free-running mode from xtal\n");
	return 0;
}
...
static void si5324_xtal_unprepare(struct clk_hw *hw)
{
	struct si5324_driver_data *drvdata =
		container_of(hw, struct si5324_driver_data, xtal);
	si5324_dbg("si5324_xtal_unprepare - nop\n");
}
...
static int si5324_clkout_prepare(struct clk_hw *hw)
{
	...
	si5324_dbg("si5324_clkout_prepare for hwdata->num=%d - NOP\n",hwdata->num);
#if 0 //clock always enabled for xilinx vphy operation
...
static void si5324_clkout_unprepare(struct clk_hw *hw)
{
	...
	si5324_dbg("si5324_clkout_unprepare for hwdata->num=%d - NOP\n",hwdata->num);
#if 0 //do not disable clocks for xilinx vphy operation
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (variant > si5328) {
		dev_err(&client->dev, "si53xx device tree entry do not indicate \
                               presence of Si5319 or Si5324 or Si5328.\n");
		return -ENODEV;
	}
	...
	for (n = 0; n < 3; n++) { //clk_in
		sprintf(inst_names[n], "%s_%d", si5324_input_names[n], instance);
	}
	sprintf(inst_names[3], "%s_%d", si5324_pll_name, instance); //pll
	for (n = 0; n < 2; n++) { //clkout
		sprintf(inst_names[n+4], "%s_%d", si5324_clkout_names[n], instance);
	}
